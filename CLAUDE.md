...
- **A table is a screen of its own on the client too.** `client/src/features/` holds one folder per
  table — `employees/`, `roles/`, `countries/`, `departments/` — each with its own page, table,
  `api.ts`, `types.ts` and query hook, so a screen can grow its own filters or columns without
  touching the others. No page takes the resource as a prop; see `client/CLAUDE.md` → Structure.
- **The API contract is duplicated on purpose.** The server DTOs
  (`server/src/employees/dto/`, `server/src/filters/dto/`), the three lookup entities and the
  `types.ts` in each of the four `client/src/features/*` folders are
  ...
  `GET /api/{roles,countries,departments}` return — the last one once per feature folder, not as a
  shared row type; the write DTOs have no client-side twin, and adding one would be dead code
  until a page posts.
